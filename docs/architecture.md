...
- A malicious frontend deployment: the browser loads JS from the server, so a compromised server could serve code that exfiltrates keys. Mitigations: CSP, SRI, code signing, and eventually self-hosting.
